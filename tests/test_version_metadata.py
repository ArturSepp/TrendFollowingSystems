...
`pyproject.toml`, `CITATION.cff` and the `@software` BibTeX entry in `README.md` are three
hand-maintained copies of one fact, and updating one leaves no trace in the others. They had
drifted before 1.0.4: `CITATION.cff` said 1.0.0 against a published 1.0.3, so a reader following
the citation file would have cited a release three patches behind the code that produced the
paper's tables. For a replication package that is a provenance defect rather than a typo.
